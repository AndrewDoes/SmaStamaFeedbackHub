...
export default function ModerationHubPage() {
  ...
  const resolveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Tanda pelanggaran berhasil diselesaikan dan dihapus");
    }
  });

  const getCategoryName = (cat: number) => {
    switch (cat) {
      case 0: return "Fasilitas";
      case 1: return "Akademik";
      case 2: return "Kesiswaan";
      case 3: return "Kantin";
      case 4: return "Pelaporan";
  ...
  return (
    <div className="w-full px-4 md:px-10 py-8 flex flex-col h-[calc(100vh-64px)]">
      ...
      <nav className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-brand-text-body/30 mb-4 shrink-0">
        ...
        <span>/</span>
        <span className="text-brand-error">Pusat Moderasi</span>
      </nav>
      ...
      <header className="mb-8 flex flex-col lg:flex-row lg:items-end justify-between gap-6 shrink-0">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-black text-brand-text-main tracking-tighter">
              Moderasi<span className="text-brand-error"> </span>Audit
            </h1>
          </div>
          <p className="text-brand-text-body/60 text-sm font-medium">Sistem keamanan aktif • {flaggedItems?.length || 0} tanda prioritas menunggu tinjauan.</p>
        </div>
      </header>
        ...
        <div className="flex-1 min-h-0 bg-brand-surface rounded-[32px] border border-brand-error/10 shadow-premium flex flex-col overflow-hidden">
            ...
            <table className="w-full text-left border-separate border-spacing-0 min-w-[900px] hidden md:table">
              <thead className="sticky top-0 z-20">
                <tr className="bg-brand-background/80 backdrop-blur-md">
                  <th className="pl-10 pr-6 py-6 text-[10px] uppercase tracking-[0.2em] font-black text-brand-text-body/40 border-b border-brand-error/5">Pengguna Ditandai</th>
                  <th className="px-6 py-6 text-[10px] uppercase tracking-[0.2em] font-black text-brand-text-body/40 border-b border-brand-error/5">Konten Melanggar</th>
                  <th className="px-6 py-6 text-[10px] uppercase tracking-[0.2em] font-black text-brand-text-body/40 border-b border-brand-error/5 text-center">Kategori</th>
                  <th className="pl-6 pr-10 py-6 text-[10px] uppercase tracking-[0.2em] font-black text-brand-text-body/40 border-b border-brand-error/5 text-right">Aksi Moderasi</th>
                </tr>
              </thead>
                ...
                {(!flaggedItems || flaggedItems.length === 0) ? (
                  <tr>
                    <td colSpan={4} className="py-24 text-center">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest text-brand-text-body/20">Tidak ada tanda pelanggaran yang aktif</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    >
                      <td className="pl-10 pr-6 py-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-bold text-brand-text-main text-sm leading-tight">{item.authorName || "Kontributor"}</p>
                            <p className="text-[9px] font-medium text-brand-text-body/30 tracking-tight">
                              {new Date(item.createdAt).toLocaleDateString('id-ID', { month: 'short', day: 'numeric' }).replace(/\//g, '-')} • {new Date(item.createdAt).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                            </p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 max-w-[400px]">
                        <div className="p-3 bg-brand-background rounded-2xl border border-brand-error/5 group-hover:border-brand-error/20 transition-all">
                          ...
                          <p className="text-[10px] text-brand-text-body/60 line-clamp-1">"{item.content}"</p>
                        </div>
                      </td>
            ...
            <div className="md:hidden flex flex-col divide-y divide-brand-error/[0.03]">
               {(!flaggedItems || flaggedItems.length === 0) ? (
                  <div className="py-24 text-center">
                     <p className="text-[10px] font-black uppercase tracking-widest text-brand-text-body/20">Tidak ada tanda pelanggaran aktif</p>
                  </div>
               ) : (
                  flaggedItems.map((item: FeedbackDto) => (
                    <div key={item.id} className="p-6 space-y-4 hover:bg-brand-error/[0.01]">
                       <div className="flex justify-between items-start">
                          <div className="flex items-center gap-3">
                             ...
                             <div>
                                <p className="font-bold text-brand-text-main text-xs">{item.authorName || "Kontributor"}</p>
                          ...
                          <span className="px-2 py-0.5 bg-brand-error/10 text-brand-error text-[7px] font-black uppercase rounded">Ditandai</span>
                       </div>
                       ...
                       <div className="p-3 bg-brand-background rounded-xl border border-brand-error/5">
                          ...
                          <p className="text-[10px] text-brand-text-body/60 line-clamp-2">"{item.content}"</p>
                       </div>
          ...
          <div className="p-4 border-t border-brand-error/5 bg-brand-error/[0.01] flex justify-between items-center shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-black uppercase tracking-widest text-brand-error/60">Tinjauan Prioritas Diperlukan</span>
            </div>
            <p className="text-[9px] font-black uppercase tracking-widest text-brand-text-body/20">Audit keamanan internal aktif</p>
          </div>
        </div>
      )}
    </div>
